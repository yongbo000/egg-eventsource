...
class EventStream {
  ...
  handler(ctx) {
    ...
    this.clients[clientId].__id = clientId;

    ctx.logger.info('[egg-eventsource] there is a new client joined, clientId:%s, count:%s', clientId, this.count);
    const closeCallback = cid => {
      return () => {
        this.deleteClient(cid);
        ctx.logger.info('[egg-eventsource] client closed, clientId:%s, count:%s', cid, this.count);
      };
    };
    const closeFn = closeCallback(clientId);
    ctx.req.on('close', closeFn);
    ctx.res.on('close', closeFn);
    ctx.res.on('error', e => ctx.logger.error(e));
  }

  _write(client, event, data) {
    const ok = client.write(`event: ${event}\ndata: ${typeof data === 'string' ? data : JSON.stringify(data)}\n\n`);
    if (!ok && !client.__bindDrain) {
      this.logger.info('[egg-eventsource] wait for drain');
      client.once('drain', () => {
        client.write('\n');
        client.__bindDrain = false;
      });
      client.__bindDrain = true;
    }
  }
      ...
      for (const clientId of clientIds) {
        ...
        try {
          if (fn) {
            ...
            this._write(client, event, data);
          }
        } catch (e) {
          this.logger.info('[egg-eventsource] clientId:%s, some exception happend, error: %j', clientId, e);
